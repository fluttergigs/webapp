...
<script lang="ts" setup>
  ...
  const snippetId = ref(Array.isArray(route.params.id) ? route.params.id[0] : route.params.id);

  const {
    data: snippet,
    error,
    status,
  } = await useFetch<Snippet>(
    `${useRuntimeConfig().public.apiBaseUrl}${Endpoint.fluppetsById(snippetId.value as string)}`,
    {
