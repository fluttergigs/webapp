...
<template>
  <main>
    <client-only>
      <div v-if="pending" class="flex items-center justify-center h-full p-52">
      ...
      <div v-else class="w-full">
        <section class="relative w-full">
          <div class="md:x-20 container mx-auto px-4 py-6 md:py-14">
            <CompanyLogo :company="company" class="absolute left-[10px] top-[6px] md:left-16 md:top-18" size="3xl" />
          </div>
        </section>
          ...
          <section class="bg-white">
            <div class="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div class="my-2 md:my-0 flex flex-wrap justify-center items-center space-x-3 space-y-1 md:space-y-0">
                <UButton v-if="useJobActions().jobBelongsToCompany(company)" color="white" icon="i-heroicons-pencil"
                  label="Edit job offer" size="xl" square variant="solid"
                  @click="useCompanyActions().handleJobEdit(jobOffer)" />

                <UButton color="white" icon="i-heroicons-share" label="Share job offer" size="xl" square variant="solid"
            ...
            <div class="my-4 flex flex-wrap gap-4 items-center">
              ...
              <div v-if="useFeatureFlags().isEnabled(AvailableFlags.companiesList)" class="flex items-center space-x-1">
          ...
          <section class="my-4 flex flex-col gap-2 gap-x-16 font-normal md:flex-row">
            <div class="flex w-full flex-col space-y-10 md:w-4/6">
              <div class="space-y-6 md:space-y-10 font-medium text-gray-900">
                <p v-if="useFeatureFlags().isEnabled(AvailableFlags.companiesList)" class="leading-10">
                  {{ company?.description }}
                </p>

                <USeparator />

                <article class="leading-[48px] md:leading-[40px]" v-html="jobOffer?.description?.isMarkdown()
            ...
            <div class="flex w-full flex-col space-y-9 md:w-2/6">
              <JobApplicationCtaCard :company="company" :job="jobOffer" :layout-direction="Direction.vertical" />
