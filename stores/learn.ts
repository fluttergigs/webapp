  ...
  actions: {
    async fetchLearnCategories(): Promise<void> {
      ...
      try {
        ...
        const response = await (<HttpClient>$http).get<MultiApiResponse<LearnCategory>>(`${Endpoint.learnCategories}?populate=*`);
    ...
    async fetchLearnResources(): Promise<void> {
      ...
      try {
        //@ts-ignore
        const { $http } = useNuxtApp();
        const response = await (<HttpClient>$http).get<MultiApiResponse<LearnResource>>(`${Endpoint.learnResources}?populate=*`);
